...
// Will be redefined for modules which need this functionality only
#define END_LOCAL_CALLBACK_PROCESSING(handle) {}
...
#define DIRECT_CALL_AND_FREE_HANDLE( _fctType, _server, ...)\
   DIRECT_CALL_START(_fctType, _server)\
      if(CORHANDLE_LOCAL_ADDRESS(_server))\
         ...
         fctStatus = fct( _server, __VA_ARGS__);\
   DIRECT_CALL_SUCCESS()\
      END_LOCAL_CALLBACK_PROCESSING(_server); \
      CORHANDLE_FREE(  _server);\
   DIRECT_CALL_ERROR()\
   DIRECT_CALL_END()
...
// Will be redefined for modules which need this functionality only
#define INIT_LOCAL_CALLBACK_PROCESSING(handle) {}
...
#define DIRECT_CALL_NEW( _fctType, _server, _handle, ...)\
   ...
   DIRECT_CALL_SUCCESS()\
      ...
      INIT_LOCAL_CALLBACK_PROCESSING( *_handle); \
   DIRECT_CALL_ERROR()\
